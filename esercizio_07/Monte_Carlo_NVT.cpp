...
int main() {
  ...
  for (int iblk = 1; iblk <= nblk; ++iblk) // Simulation
  {
    ...
    for (int istep = 1; istep <= nstep; ++istep) {
      ...
      if (instantoutput == 1) // Print instantaneous U/N and P
        Output();
...
void Output(void) {
  ...
  instantE << walker[iv] / (double)npart + vtail << endl;
  instantE.close();

  instantP.open("output.pressure", ios::app);
  instantP << rho * temp + (walker[iw] + (double)npart * ptail) / vol << endl;
  instantP.close();
}
...
void Welcome() {
  ...
  if (restart == 1)
    cout << "Read initial configuration from file config.final ";
  else
    cout << "Read initial configuration from file config.0 ";
  cout << endl << endl;
...
void Averages(int iblk) // Print results for current block
{
  ...
  // Pressure
  stima_pres =
      rho * temp + (blk_av[iw] / blk_norm + ptail * (double)npart) / vol;
  ...
  for (int ibin = 0; ibin < nbins; ++ibin) {
    ...
    r = dr * ibin;
    dvol = 4 * pi * (pow(r + dr, 3) - pow(r, 3)) / 3;
    ...
    Gofr << iblk << " " << ibin + 1 << " " << r << " ";
    Gofr << gdir << " " << glob_av[igofr + ibin] / (double)iblk << " "
         << err_gdir << " " << endl;

    if (iblk == nblk){ // Final g(r)
      Gave << ibin << " " << r << " ";
      Gave << glob_av[igofr + ibin] / (double)iblk << " ";
      Gave << err_gdir << " " << endl;
    }
  }
