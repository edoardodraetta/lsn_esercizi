...
double external_potential(double val) {
  ...
  return val*val*val*val - 2.5*val*val; // NEW
}
...
double external_potential_prime(double val) {
  ...
  return 4*val*val*val - 5*val; // !! NEW
}
...
// TODO(): implement your trial wavefunction
double variationalWaveFunction(double v) {
  // return exp(-0.5*v*v);
  if (MODE == true) return 1.0;
  else if (MODE == false){ // !! NEW
    double a, b, vmm, vpm, s2;
    double mu, sigma;
    ...
    vmm = v - mu;
    vpm = v + mu;
    a = exp(- vmm*vmm / (2*s2)); // left gaussian
    b = exp(- vpm*vpm / (2*s2)); // right gaussian

    return - (a*vmm + b*vpm) / s2;
  }
}
...
double variationalWaveFunction_second(double v) {
  ...
  if (MODE == 0) return 0;
    ...
    return - a/s2 - b/s2 + a*vmm*vmm/s4 + b*vpm*vpm/s4;
  }
}
